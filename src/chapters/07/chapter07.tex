...
Per un ciclo termico si definisce rendimento la quantità adimensionale 
\begin{equation}
    \eta = \frac{W}{Q_A} = \frac{Q_A + Q_C}{Q_A} = 1+ \frac{Q_C}{Q_A} = 1 - \frac{|Q_C|}{Q_A}
\end{equation}
Il rendimento è quindi la percentuale di calore assorbito che viene trasformata in lavoro.
...
\end{equation*}
Il lavoro fatto dal gas è \(W_{BC} = -\Delta U_{BC} = n c_V \left(T_2 - T_1\right)\).

Nella trasformazione \(C D\) il gas è a contatto termico con una sorgente di calore a temperatura \(T_1\). 
Il processo è analogo ad \(A B\), però ora si aumenta la presione esterna di \(d p\), il gas si comprime di una quantità \(d V\) ed aumenta la sua temperatura di \(d T\), cede \(d Q\) alla sorgente a temperatura \(T_1\) e ritorna alla temperatura \(T_1\). 
...
\begin{equation*}
    \eta = \ + \frac{Q_C}{Q_B} = 1 + \frac{n R T_1 \ln \left(V_D / V_C \right) }{n R T_2 \ln \left(V_B / V_A \right)} = 1 - \frac{T_1 \ln \left(V_C / V_D \right) }{T_2 \ln \left(V_B / V_A \right)}
\end{equation*}
...
\begin{enumerate}
    \item Nella trasfom1azione \(O A\) la miscela benzina-aria viene a aspirata nel cilindro, a pressione costante (\emph{fase di aspirazione}). 
...
Il rendimento del ciclo di Otto calcolato con la formula appena ricavata va inteso come valore massimo teorico: nei motori reali si ottengono rendimenti \(0.2 - 0.3\). 
...
Nella situazione più semplice il sistema asso il calo \(Q_0\) dalla sorgente fredda, assorbe lavoro e cede il calore \(Q_C\) a una sorgente calda: risulta sempre \(|Q_C| > Q_0\).

Si definisce \emph{efficienza} o \emph{coefficente di prestazione} di un ciclo frigorifero il rapporto
\begin{equation}
    \xi = \frac{Q_0}{W}
\end{equation}
...
\begin{enumerate}
    ...
    \item \emph{non ci sono forze intermolecolari, se non durante gli urti}: gli urti sono dovuti cieè a forze repulsive a corto raggio di azione, mentre si assumono trascurabile le forze attrattive tra le molecole
    \item \emph{le dimensioni delle molecole sono molto piccole rispetto alle distanze medie tra di esse}
\end{enumerate}

Sulla base della prima ipotesi (caos molecolare), in ogni istante, considerata una qualsiasi direzione orientata \(\overrightarrow{u}\), ci sono tante molecole che hanno velocità concode a \(\overrightarrow{u}\) quante hanno velocità discorde a \(\overrightarrow{u}\); la velocità media \(\overrightarrow{v}_m\) è quindi nulla. 
...
Pertanto il numero di urti al secondo sulla parete \(y z\) è dato da \(1 /t = v_c / 2a\) e l'impulso comunicato in un secondo, pari alla componente \(x\) della forza media esercitata sulla parete da una molecola, vale in modulo:
\begin{equation*}
...
Per un gas ideale ho che \(p V = n R T\), egualiado si trova:
\begin{equation*}
...
% TODO verificare se c'è tutto (11.10 Mazzoldi 1)
